...
# Xtr = np.loadtxt('XTrain_10k.txt',delimiter=',').astype('int')
# Xts = np.loadtxt('XTest_10k.txt',delimiter=',').astype('int')
# ytr = np.loadtxt('YTrain_10k.txt',delimiter=',').astype('int')
# yts = np.loadtxt('YTest_10k.txt',delimiter=',').astype('int')

Xtr = np.loadtxt('DatasetX_10k.txt',delimiter=',').astype('int')
...
def Accuracy(X,y,ModelName):
	...
	return (A)
fh = open('Positions_10k_10.txt','w+')
fh.close()
fh = open('Weights_10k_10.txt','w+')
fh.close()
fh = open('bias_10k_10.txt','w+')
fh.close()
import os
TT_list = [10]
TrainAccuracy = []
...
for top_tags in TT_list:
	for i in np.arange(0,int(ytr.shape[1])):#int(ytr.shape[1])
		...
		fh = open('Positions_10k_10.txt','a')
		...
		model_10k.compile(loss='categorical_crossentropy', optimizer=opt)
		model_10k.summary()  
		...
		W[W==-0.0]=0.0
		b[b==-0.0]=0.0
		f=open('Weights_10k_10.txt','ab')
		np.savetxt(f,W,delimiter=',',fmt='%1.4f')
		f.close()
		f=open('bias_10k_10.txt','ab')
